...
use crate::{api::{AppError, AUTH_COOKIE}, crypto, user::fetch_and_cache_github_user, AppState};
...
pub async fn auth_middleware(
    ...
    let github_id = match maybe_id {
        Some(id) => id,
        None => fetch_and_cache_github_user(&token, &client, &encrypted_token, &token_ids).await?.id,
    };
