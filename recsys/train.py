...
DB_PARAMS = "dbname=your_db user=postgres password=secret host=localhost"

def fetch_data():
    conn = psycopg2.connect(DB_PARAMS)
    cur = conn.cursor()

    # 1. Map Projects to contiguous indices
    cur.execute("SELECT id, language_id, tag_ids FROM projects")
    ...
    # 2. Map Users and Fetch Interactions
    cur.execute("SELECT user_id, project_id FROM interactions WHERE type IN ('view', 'like')")
...
def create_batches(user_histories, item_records, project_to_idx, batch_size=32):
    ...
    for i in range(0, len(user_ids), batch_size):
        ...
        u_ids, histories, pos, neg = [], [], {'ids':[], 'tags':[], 'langs':[]}, {'ids':[], 'tags':[], 'langs':[]}
...
if __name__ == "__main__":
    ...
    payload = {
        'model': model,
        'idx_to_project': idx_to_p,
        'project_to_idx': p_to_idx
    }
